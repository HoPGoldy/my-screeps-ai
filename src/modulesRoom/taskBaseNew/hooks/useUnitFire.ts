import { TaskUnitInfo } from '../types'
 ...
 */
export const useUnitFire = function (getCreepInfos: () => Record<string, TaskUnitInfo>) {
    /**
     * 开除某个 creep
     * 被开除的 creep 将会继续工作，直到死掉后将不再继续孵化
     */
    const fireCreep = function (creepName: string): void {
        const creepInfos = getCreepInfos()
    ...
    const haveCreepBeenFired = function (creepName: string): boolean {
        const unitInfo = getCreepInfos()[creepName]
