...
import { RemoteInfo, RemoteShowInfo } from './types'
import RoomAccessor from '../RoomAccessor'
import { GetName } from '../spawn/nameGetter'
import { RemoteMemory } from './types'
...
export default class RoomShareController extends RoomAccessor<RemoteMemory> {
     ...
     */
    public disableRemote(remoteRoomName: string, sourceId: Id<Source>, disableTick: number = 1500) {
    ...
    private setRemoteInfo(remoteRoomName: string, sourceId: string, info: RemoteInfo) {
        ...
        else this.memory[remoteRoomName][sourceId] = {
     ...
     * @returns 存放建筑
     */
    public getRemoteEnergyStore(remoteRoomName: string, sourceId: string, ignoreCache: boolean = false): StructureWithStore {
     ...
     */
    public claim(targetRoomName: string, signText: string = ''): OK {
