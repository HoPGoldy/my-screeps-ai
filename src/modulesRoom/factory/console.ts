...
export default class FactoryConsole extends Room {
    ...
    public fsetchain(...depositTypes: DepositConstant[]): string {
        ...
        if (result === OK) {
            ...
            const command = colorful(`Game.rooms.${this.name}.factory.setlevel`, 'yellow')
            const help = colorful(`Game.rooms.${this.name}.factory.help`, 'yellow') + '()'
    ...
    public fremove(): string {
        ...
        if (actionResult === OK) {
            const stopCommand = colorful(`delete Game.rooms.${this.name}.memory.factory.remove`, 'yellow')
