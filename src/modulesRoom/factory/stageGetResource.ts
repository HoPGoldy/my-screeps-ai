...
export default class StageGetResource extends FactoryBase {
        ...
        if (source === STRUCTURE_TERMINAL && this.room.terminal) {
            if (this.room.terminal.store[resType] < amount) {
                ...
                if (!this.inBlacklist(resType)) {
                    ...
                    this.log(`发现底物不足，进行拆分：${resType} ${amount}`, 'yellow', true)
                }
                // 缺少的是基础资源，等一等
                else this.gotoBed(100, `缺少 ${resType}*${amount}`)
                // this.log(`合成暂停，缺少 ${resType}*${needAmount}`, 'yellow')
                return
            }
        }
        // 能量太少了就先不干
    ...
    private hasEnoughEnergy(): boolean {
        if (this.room.storage && this.room.storage.store[RESOURCE_ENERGY] >= ENERGY_LIMIT) return true
