 ...
 * 拓展模块的用户访问接口
 */
export class RemoteConsole extends Room {
    /**
     * 拓展新外矿
     */
     ...
     * 移除外矿
     */
    public rremove (remoteRoomName: string): string {
        let logs = `[${this.name} 外矿] `

        const result = this.remote.remove(remoteRoomName)
     ...
     */
    public rshow (): string {
        const remoteList = this.remote.getRemoteList()
