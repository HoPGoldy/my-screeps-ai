...
const getRoomCost = function (room: Room, structurePlacePlan: AutoPlanResult[]): CostMatrix {
    ...
    for (let y = 0; y < 50; y++) {
        for (let x = 0; x < 50; x++) {
            ...
            const weight = tile === TERRAIN_MASK_WALL ? 255 : (tile === TERRAIN_MASK_SWAMP ? 30 : 10)
    ...
    for (let i = 0; i < 8; i++) {
        ...
        Object.keys(currentLevelLayout).forEach((structureType: BuildableStructureConstant) => {
            // 遍历该建筑下的所有预放置点位
            currentLevelLayout[structureType].forEach((pos: RoomPosition) => {
                if (!pos) return

                // 如果是道路，就降低 cost
                if (structureType === STRUCTURE_ROAD) matrix.set(pos.x, pos.y, 1)
                // 如果不是 rampart 和 container 就不能走
