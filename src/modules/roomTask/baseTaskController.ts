export default class TaskController implements InterfaceTaskController {
    /**
     * 本任务对象所处的房间名
     */
     ...
     * @param opt 配置项
     */
    public addTask(task: RoomTask<string>, opt: AddTaskOpt = { dispath: true }) {
     ...
     */
    public getTask<T extends string>(taskKey: number): RoomTask<T> | undefined {
        if (!taskKey) return undefined
        return this.tasks.find(task => task.key === taskKey) as RoomTask<T>
    }
     ...
     */
    private setTaskUnit(task: RoomTask<string>, unit: Creep): void {
        if (!task || !unit) return
     ...
     */
    private removeTaskUnit(task: RoomTask<string>, unit?: Creep): void {
     ...
     */
    private dispatchCreep(creep: Creep): RoomTask<string> {
     ...
     */
    public hasTask(taskIndex: number | string): boolean {
        // 用任务类型判断
    ...
    public removeTask(taskIndex: string): OK | ERR_NOT_FOUND
    public removeTask(taskIndex: number | string): OK | ERR_NOT_FOUND {
     ...
     */
    public getUnitTaskType<T extends string>(creep: Creep): RoomTask<T> {
     ...
     */
    private getUnit(filter?: (info: TaskUnitInfo, creep: Creep) => boolean): Creep[] {
        const units: Creep[] = []
